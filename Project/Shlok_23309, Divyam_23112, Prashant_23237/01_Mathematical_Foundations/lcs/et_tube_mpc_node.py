#!/usr/bin/env python3
"""
Event-Triggered Tube MPC Controller Node for ROS2/Gazebo.

ET-Tube MPC: Only solves optimization when tracking error exits robust invariant set.
Uses Schmitt trigger: ||p_true - p_bar|| > 0.15 for 3 consecutive steps.
...
Publications:
    ...
    /drone/et_trigger (std_msgs/String): Event trigger notifications

Key difference from Tube MPC:
    - Solves QP only on event triggers (reduces computation)
    - Uses nominal trajectory propagation between triggers
    - Logs trigger events for analysis
"""
...
class ETTubeMPCNode(Node):
    """ROS2 node for Event-Triggered Tube MPC control."""
    ...
    def __init__(self):
        ...
        self.declare_parameter('trigger_threshold', 0.15)  # Position error threshold (m)
        self.declare_parameter('consecutive_threshold', 3)  # Consecutive steps to trigger
        ...
        self.get_logger().info(f'  Trigger threshold: {trigger_threshold}m, '
                              f'Consecutive: {consecutive_threshold}')
        self.mpc = ETTubeMPC(
            horizon=horizon,
            dt=dt,
        ...
        self.trigger_pub = self.create_publisher(String, '/drone/et_trigger', 10)
    ...
    def _control_loop(self):
        ...
        # Solve MPC (with event trigger logic inside)
        ...
        if result['trigger']:
            ...
            if self.trigger_count % 10 == 0:
                ...
                self.get_logger().info(f'Event trigger #{self.trigger_count} at step {self.total_steps} '
    ...
    def destroy_node(self):
        ...
        if self.total_steps > 0:
            rate = 100.0 * self.trigger_count / self.total_steps
            self.get_logger().info(f'Final statistics: {self.trigger_count}/{self.total_steps} '
