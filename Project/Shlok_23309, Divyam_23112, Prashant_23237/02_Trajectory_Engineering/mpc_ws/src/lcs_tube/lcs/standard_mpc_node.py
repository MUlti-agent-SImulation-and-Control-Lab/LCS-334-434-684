#!/usr/bin/env python3
"""
Standard MPC Controller Node for ROS2/Gazebo.

Baseline controller without tube constraints.
Solves MPC optimization at every control step.
...
from lcs.mpc_solvers import StandardMPC


class StandardMPCNode(Node):
    """ROS2 node for Standard MPC control."""
    
    def __init__(self):
        super().__init__('standard_mpc_node')
        ...
        self.declare_parameter('waypoint_threshold', 0.1)  # Distance to consider waypoint reached
        ...
        self.get_logger().info(f'Initializing Standard MPC: horizon={horizon}, dt={dt}')
        self.mpc = StandardMPC(
            horizon=horizon,
            dt=dt,
        ...
        self.get_logger().info('Standard MPC Node initialized')
    ...
    def _build_accel_msg(self, u: np.ndarray) -> Accel:
        ...
        msg.linear.y = u[1]
        msg.linear.z = u[2]
        # Angular acceleration set to zero (attitude controller handles orientation)
        msg.angular.x = 0.0
        msg.angular.y = 0.0
        msg.angular.z = 0.0
        return msg
