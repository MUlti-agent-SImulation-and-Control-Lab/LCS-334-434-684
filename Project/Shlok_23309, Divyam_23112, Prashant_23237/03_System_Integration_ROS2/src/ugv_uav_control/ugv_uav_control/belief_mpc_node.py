...
Component: Belief-Space MPC Node
Purpose: Implements the 'Information-Aware' UGV controller. 
         Minimizes path tracking error while simultaneously reducing estimation 
         uncertainty (Covariance Trace) by prioritizing UAV observability.
Inputs: /waypoints (geometry_msgs/PoseStamped), /belief_state (PoseWithCovarianceStamped)
Outputs: /cmd_vel (geometry_msgs/TwistStamped)
"""
import rclpy
...
class BeliefMPCNode(Node):
    def __init__(self):
        ...
        self.w_head = 10.0       # Heading error (Increased for better tracking)
    ...
    def belief_callback(self, msg):
        ...
        self.get_logger().info(f'Belief Received: x={self.mu[0]:.2f}, y={self.mu[1]:.2f}', once=True)
    ...
    def waypoint_callback(self, msg):
        ...
        solve_time_ms = (self.get_clock().now() - start_time).nanoseconds / 1e6
        ...
        if result.success:
            ...
            # Professional Solve Time Log (Sync with report)
            self.get_logger().info(f'[MPC] Optimized in {solve_time_ms:.1f}ms | Result: Success')
        else:
        ...
        self.get_logger().info(f'UGV CMD Published: v={v_cmd:.3f}, w={w_cmd:.3f}')
