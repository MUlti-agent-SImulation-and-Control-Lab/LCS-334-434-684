...
class SimLogger:
    ...
    def log(self, t, v, ey, a, yaw_dot, dist, s_time):
        self.t.append(t)
        self.v_act.append(v)
...
ax_anim.set_title("Live Vehicle Simulation (Nominal SMPC)")
...
def update(frame):
    ...
    for i, veh in enumerate(all_vehicles):
        ...
        if veh.id == 'Ego':
            ...
            logger.log(current_time, veh.v, veh.ey, a_cmd, yaw_rate_cmd, min(dists), solve_duration)
...
# ==========================================
print("Rendering Nominal MPC simulation. This will populate the logger...")
...
print("Animation saved successfully as 'nominal_smpc_simulation.gif'!")

# Close the animation figure so we can cleanly plot the dashboard
plt.close(fig_anim)
...
def generate_dashboard(log):
    ...
    fig_dash.suptitle("SMPC Simulation Analysis: Command vs. Actual", fontsize=16, fontweight='bold')
    
    t = log.t

    # Plot 1: Velocity
    axs[0, 0].plot(t, log.v_act, color='blue', lw=2)
    axs[0, 0].set_title("Ego Velocity Profile")
    axs[0, 0].set_ylabel("Velocity (m/s)")

    # Plot 2: Lateral Deviation
    axs[0, 1].plot(t, log.ey_act, color='green', lw=2)
    axs[0, 1].axhline(y=config.LANE_WIDTH, color='k', ls='--', alpha=0.3)
    axs[0, 1].axhline(y=-config.LANE_WIDTH, color='k', ls='--', alpha=0.3)
    ...
    plt.savefig('smpc_performance_dashboard.png')
    print("Dashboard saved successfully as 'smpc_performance_dashboard.png'!")
    plt.show()
